...
function randomNomor(min, max = null) {
        ...
        let reply1 = (teks) => {
			deff.sendMessage(from, teks, text, { thumbnail: Rfake, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"DON'T CLICK HERE",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/p/6285719116912`}}})
		}
		let reply = (teks) => {
...
if (isGroupAdmins) return reply("Admin mah bebas ye kan😎");
		...
		deff.sendMessage(`6285719116912@s.whatsapp.net`, `wa.me/${sender.split("@")[0]}\n telah mengirim Virtex ❗`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "BOT WHATSAPP",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./gambar/thumb.jpg'),sourceUrl:"https://wa.me/6285719116912"}}})            
        }, 7000)
        setTimeout( () => {
        deff.sendMessage(from, '_1 Detik lagi..._', text, {quoted: fkontak})
        }, 6000)
...
case 'requestdev':
      ...
      deff.sendMessage(`6285719116912@s.whatsapp.net`,`*Request :* ${v}`, text) //Jangan diubah bang
      break
case 'request':
...
case 'report':
      ...
      deff.sendMessage('6285719116912@s.whatsapp.net',`*Bug Report:* ${v}`, text) //jangan diubah ya adik²
      break
//=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×==×=×=×=×=×=×=×=×=×==×=×=×=×=×=×=×=×=×==×=×=×=×=×=×=×=×=×=×𝘤
...
`
  buttons = [{ buttonId: `${prefix}owner`, buttonText: { displayText: '𝕭𝖔𝖙 • Andii ༢࿔ྀ' }, type: 1 }, { buttonId: `${prefix}rules`, buttonText: { displayText: '❥ࣲ፝֟͜͡◔⃟➣ ᴘʀɪᴠᴀᴛᴇ 𝕭𝕯ᴮᴷᴾ ㊧' }, type: 1 },]
...
case 'broadcast3':
            ...
            if (args.length < 1) return reply('teks?')
             ...
             for (let _ of anu) {
             deff.sendMessage(_.jid, { contentText: `*🔖 BROADCAST [ • 𝕭𝖔𝖙 • ] 🔖*\n\n${body.slice(4)}`, footerText: '© 𝕭𝖔𝖙', buttons: [{ buttonId: `${prefix}command`, buttonText: { displayText: 'MENU' }, type: 1 }, { buttonId: `${prefix}owner`, buttonText: { displayText: 'OWNER' }, type: 1 }], headerType: 6, locationMessage: { degreesLatitude: 0, degreesLongitude: 0, jpegThumbnail: bc2 }}, 'buttonsMessage')
}
             reply('Suksess broadcast')
}
break
...
//=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×=×==×=×=×=×=×=×=×=×=×==×=×=×=×=×=×=×=×=×==×=×=×=×=×=×=×=×=×=×			
     ...
     deff.sendMessage(`${target}`, `─────「 *EROR DITEMUKAN* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "BOT WHATSAPP",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./gambar/thumb.jpg'),sourceUrl:"https://wa.me/6285719116912"}}})
	}
    }
    }
    }
